...
// $twitter_options['widget_fields']['encode_utf8'] = array('label'=>'UTF8 Encode:', 'type'=>'checkbox', 'default'=>true);
...
function twitter_messages($username = '', $num = 1, $list = false, $update = true, $linked	= '#', $hyperlinks = true, $twitter_users = true, $encode_utf8 = true) {
	...
	$feed = new SimplePie();
	$feed->set_feed_url('http://twitter.com/statuses/user_timeline/'.$username.'.rss');
...
//					$link = $message['link'];
		...
		if($update) {
			...
			echo sprintf( __('%s', 'twitter-for-wordpress'),' <span> ' . $h_time . '</span>' );
		}
					echo '</div><div class="decorator"></div></li>';
...
// Link discover stuff
	...
	function parseLink($data){
		return '<a href="'.$data[0].'" class="twitter-link">'.((strlen($data[0])>23) ? substr($data[0],0,20).'...' : $data[0]).'</a>';
	}
...
function hyperlinks($text) {
	...
	$text = preg_replace_callback('/\b([a-zA-Z]+:\/\/[\w_.\-]+\.[a-zA-Z]{2,6}[\/\w\-~.?=&%#+$*!]*)\b/i','parseLink', $text);
...
function widget_twitter_init() {
	...
	function widget_twitter($args, $number = 1) {
		...
		twitter_messages($item['username'], $item['num'], true, $item['update'], $item['linked'], $item['hyperlinks'], $item['twitter_users'], true);
		echo $after_widget;
	...
	?>
		<div class="wrap">
			<form method="POST">
				<h2><?php _e('Twitter Widgets', 'invent'); ?></h2>
				<p style="line-height: 30px;"><?php _e('How many Twitter widgets would you like?', 'invent'); ?>
